...
# from tools.utils import subdivide_by_step, smooth_route, subdivide_route, map_en_route_to_veh_route, compute_walk_point, clip_en_route, project_point_to_segment, match_route
from tools.utils import *


def find_ego_lanes(bev_lanes):
    """Select the closest left and right lane boundaries in BEV coordinates."""
    left_lane_diffs = []
    right_lane_diffs = []

    for lane in bev_lanes:
        if len(lane) < 2:
            continue

        xs = torch.tensor([pt[0] for pt in lane])
        base_x = xs[-1]  # Assume the last point is nearest to the vehicle.

        if base_x < 0:
            left_lane_diffs.append((base_x.item(), lane))
        else:
            right_lane_diffs.append((base_x.item(), lane))

    ego_lanes = []
    if left_lane_diffs:
        left_lane_diffs.sort(key=lambda x: x[0])
        ego_lanes.append(left_lane_diffs[-1][1])
    if right_lane_diffs:
        right_lane_diffs.sort(key=lambda x: x[0])
        ego_lanes.append(right_lane_diffs[0][1])
